...
### overview (차량 전체)
- 차량 전체가 보이는 사진 (전면/후면/측면)
- 휠이 차량에 장착된 전체 모습
- 정비소/작업장에서 차량 전체 촬영

### before (복원 전) - 손상된 상태
반드시 다음 특징 중 하나 이상이 보여야 함:
- 스크래치, 긁힘, 찍힘이 표면에 보임
- 부식, 녹, 산화 흔적
- 먼지, 때, 오염이 묻어 있음
- 표면이 거칠거나 갈라짐
...
1. **광택이 나면 무조건 after인가?** → 아니요!
   - 작업 환경(코팅 부스, 스프레이)에서의 광택 휠 = process
   - 전시/촬영 상태의 광택 휠 = after

2. **before vs after 구분 핵심**
   - 표면에 손상/오염이 있으면 → before
   - 표면이 완벽하고 손상이 없으면 → after

3. **process 우선 판단**
   - 작업 도구, 스프레이, 마스킹이 보이면 → process (광택 여부 무관)
...
|-------------|-------------|
| 휠 표면에 긁힘, 먼지 | before |
| 타이어 장착된 더러운 휠 | before |
| 광택나지만 스프레이 분사 중 | process |
| 코팅 부스 안의 깨끗한 휠 | process |
| 광택나고 손상 없는 완성 휠 | after |
| 캡/로고 장착된 완벽한 휠 | after |
| 차량에 장착된 전체 모습 | overview |

출력 형식 (JSON만, 다른 텍스트 없이):
{
  "phase": "after",
  "phaseConfidence": 0.95,
  "phaseReason": "표면에 광택이 나고 손상이 전혀 없는 완성 상태"
}`;
